...
namespace SelfhelpOrderMgr.DAL
{
	...
	public partial class T_BONUSDAL
	{
		...
		public void Add(SelfhelpOrderMgr.Model.T_BONUS model)
		{
			...
			strSql.Append("BID,CheckDate,FLAG,fareaName,Frealareacode,FrealAreaName,udate,ptype,cnt,auditby,auditflag,FAREACODE,auditdate,Fdbcheckflag,Fdbcheckdate,FPostBy,FPostDate,FPostFlag,FDbCheckBY,FCheckFlag,TypeFlag,SubTypeFlag,fAMOUNT,DType,TargetExaminerBy,MainStatus,Remark,ApplyBy,Applydt,Crtby,crtdt,CHECKBY");
			...
			SqlParameter[] parameters = {
						...
						new SqlParameter("@DType", SqlDbType.VarChar,1) ,
			...
			parameters[20].Value = model.TypeFlag;
			parameters[21].Value = model.SubTypeFlag;
			parameters[22].Value = model.fAMOUNT;
			parameters[23].Value = model.DType;
			parameters[24].Value = model.TargetExaminerBy;
			parameters[25].Value = model.MainStatus;
			parameters[26].Value = model.Remark;
			parameters[27].Value = model.ApplyBy;
			parameters[28].Value = model.Applydt;
			parameters[29].Value = model.Crtby;
			parameters[30].Value = model.crtdt;
			parameters[31].Value = model.CHECKBY;
		...
		public bool Update(SelfhelpOrderMgr.Model.T_BONUS model)
		{
			...
			parameters[30].Value = model.Crtby;
			parameters[31].Value = model.crtdt;
			parameters[32].Value = model.CHECKBY;
		...
		public SelfhelpOrderMgr.Model.T_BONUS GetModel(string BID)
		{
			...
			StringBuilder strSql = new StringBuilder();
			strSql.Append("select BID, CheckDate, FLAG, fareaName, Frealareacode, FrealAreaName, udate, ptype, cnt, auditby, auditflag, FAREACODE, auditdate, Fdbcheckflag, Fdbcheckdate, FPostBy, FPostDate, FPostFlag, FDbCheckBY, FCheckFlag, TypeFlag, SubTypeFlag, fAMOUNT, DType, TargetExaminerBy, MainStatus, Id, Remark, ApplyBy, Applydt, Crtby, crtdt, CHECKBY  ");
			...
			if (ds.Tables[0].Rows.Count > 0)
			{
				...
				model.TargetExaminerBy = ds.Tables[0].Rows[0]["TargetExaminerBy"].ToString();
				if (ds.Tables[0].Rows[0]["MainStatus"].ToString() != "")
				{
					model.MainStatus = int.Parse(ds.Tables[0].Rows[0]["MainStatus"].ToString());
				}
