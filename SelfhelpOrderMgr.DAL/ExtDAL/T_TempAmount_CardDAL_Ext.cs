    ...
    public partial class T_TempAmount_CardDAL
    {
        public List<T_TempAmount_Card> GetSearchCardAmount(string strFCrimeCode, string strFCrimeName, string strAreaName, string cardStatus,string otherWhere)
        {
            ...
            using (IDbConnection conn = new SqlConnection(SqlHelper.getConnstr()))
            {
                ...
                sql = "select a.fcrimecode,b.fname,c.fname fareaName,a.cardflaga,a.SecondaryBankCard as bankaccno,a.amounta,a.amountb,a.amountc,a.amountD,(a.amounta+a.amountb+a.amountc) fmoney,AccPoints from t_criminal_card a,t_criminal b,t_area c where a.fcrimecode=b.fcode and b.fAreaCode=c.fcode and isnull(b.fflag,0)=0";
                if ("" != strFCrimeCode)
                {
