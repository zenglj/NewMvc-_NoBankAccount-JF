...
namespace SelfhelpOrderMgr.Common
{
    public static class HttpHelper
    {
        ...
        /// <returns></returns>
        public static string HttpPostByJson(string Url, string postDataStr,string token)
        {
            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(Url);
            request.Method = "POST";
            ...
            //request.Headers.Add("Authorization", token);//添加token
            request.Headers.Add("token", token);//添加token
            ...
            {
                return $"Err|{response.StatusDescription}";
            }
            string encoding = response.ContentEncoding;
            if (encoding == null || encoding.Length < 1)
            {
