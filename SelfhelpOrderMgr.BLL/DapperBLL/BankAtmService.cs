...
namespace SelfhelpOrderMgr.BLL
{
    public class BankAtmService : BaseDapperBLL
    {
        ...
        public void GetPaymentRecordInfo(string fcrimecode, int tranStatus,string cashPwd,decimal tranMoney, out PageResult<ViewPaymentRecordExtend> rec, out ViewBankUserInfo model)
        {
            //var _s = new { FCrimeCode = fcrimecode, tranStatus = tranStatus };
            //rec = this.GetPageList<ViewPaymentRecordExtend, ViewPaymentRecordExtend>("Id",jss.Serialize(_s));
            //ViewPaymentRecordExtend row = rec.rows[0];
        ...
        public ResultInfo InsertATMOperationREC(reqAtmActionInfo reqJson, string strIp, string title)
        {
            ...
            else
            {
                ...
                if ( title == "对账")
                    ...
                    {
                        istatusFlag = -1;
                        //变动的金额为：机器上传的实时余额-电脑的记账金额
                        changeMoney = Convert.ToDecimal(reqJson.F_AMOUNT) - atm.MachineBalance;
                ...
                else if (title == "清机")
                    {
                        ...
                        if (atm.MachineBalance == Convert.ToDecimal(reqJson.F_AMOUNT))
                        {
                            istatusFlag = 1;
                            //变动余额：将减去记账的金额
                            changeMoney = 0 - atm.MachineBalance;
        ...
        public ResultInfo InsertATMOperationREC(reqAtmActionInfo reqJson,T_Criminal criminal, string strIp, string title)
        {
            string jsonWhere = this.jss.Serialize(new
            {
                IPAddr = strIp
        ...
        public ResultInfo WithdrawalOperation(decimal money, string withdrawalPassword, string fcrimecode, int tranStatus, int updateStatus, string bankResultInfo, string actionName)
        {
            ...
            this.GetPaymentRecordInfo(fcrimecode, tranStatus,withdrawalPassword,money, out rec, out model);
            if (rec.rows.Count > 0)
            {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            //else
                            {
                                ...
                                if (tranStatus != model.TranStatus)
                                {
                                    _record.BankResultInfo = bankResultInfo+"。补传";
                                ...
                                if (base.Update<T_Bank_PaymentRecord>(_record))
                                {
                                    ...
                                    //更新VCRD的出款标志
                                    base.ExecuteSql("update t_vcrd set bankflag=2,senddate=getdate() where seqno in(select vcrdSeqno from t_Bank_PaymentDetail where MainId=@MainId);" +
                                        "update t_Bank_PaymentDetail set SuccFlag=1 where MainId=@MainId;",new { MainId=_record.Id });
