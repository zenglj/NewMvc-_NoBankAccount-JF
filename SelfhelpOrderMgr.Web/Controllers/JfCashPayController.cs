...
namespace SelfhelpOrderMgr.Web.Controllers
{
    ...
    public class JfCashPayController : Controller
    {
        //
        // GET: /CashPay/
        ...
        public ActionResult Index(int id=1)
        {
            ...
            if (id == 1)//存款
            {
                saveTypes = new T_SavetypeBLL().GetModelList("typeFlag=0 and UseType=1");
            }
            else if (id == 2)//取款
            {
                saveTypes = new T_SavetypeBLL().GetModelList("typeFlag=1  and UseType=1");
            }
        ...
        public ActionResult SavePayDetail(int id = 1)
        {
            ...
            if (id == 1)
            {
                savetypes = new T_SavetypeBLL().GetModelList("typeflag=0  and UseType=1 and FCode=" + strDType);
                savePayFlag = 1;
            }
            else if (id == 2)
            {
                savetypes = new T_SavetypeBLL().GetModelList("typeflag=1  and UseType=1 and FCode=" + strDType);
                if (savetypes.Count < 0)
                {
                    ...
                    //验证积分是否够扣
                if (card.AccPoints  < Convert.ToDecimal(strFMoney))
                {
            ...
            T_JF_Vcrd vcrd = _jifenMgrService.UserCashPayJiFen(criminal, savePayFlag, Convert.ToDecimal(strFMoney), savetypes[0], strLoginName, strRemark, strApply, "", auditFlag);
        ...
        private ActionResult DelCashPayDetail(string FCode, string seqno, string strLoginName, int id)
        {
            ...
            if (id == 1)
            {
                if (vcrd.DAmount > card.AccPoints)
                {
                    return Content("Err|A账户积分已经被消费，余额不足，不能删除");
                }

            }
            //new T_VcrdBLL().SoftDeleteVcrd(FCode, Convert.ToInt32(seqno), strLoginName);
            vcrd.Flag = 1;
            vcrd.DelBy = strLoginName;
            vcrd.DelDate = DateTime.Now;
            if (_jifenMgrService.Update<T_JF_Vcrd>(vcrd))
            {
                _jifenMgrService.ExecuteSql("update t_Criminal_Card set Accpoints=Accpoints-@Accpoints where FCrimeCode=@FCrimeCode", new { Accpoints = vcrd.DAmount - vcrd.CAmount, FCrimeCode = card.fcrimecode });
                List<T_UserInfoExt> users = new T_CriminalBLL().GetUserInfo("FCode='" + FCode + "'");

                Log4NetHelper.logger.Warn("删除存取款记录,操作员：" + Session["loginUserName"].ToString() + ",删除信息 ID=" + vcrd.Id + ",录入员：" + vcrd.CrtBy + ",流水号为：" + vcrd.Vouno + ",记录创建日期:" + vcrd.CrtDate.ToString() + ",类型为：" + vcrd.DType + ",金额:" + (vcrd.DAmount + vcrd.CAmount).ToString() + ",摘要:" + vcrd.Remark);
        ...
        public ActionResult ClearPoints(string FCode,string remark)
        {
            if (string.IsNullOrEmpty(FCode))
            {
                return Content("Err|用户编号不能为空");
            ...
            T_Criminal_card card = _jifenMgrService.QueryModel<T_Criminal_card>("FCrimeCode", FCode);
            T_Criminal criminal = _jifenMgrService.QueryModel<T_Criminal>("FCode", FCode);
            T_AREA area = _jifenMgrService.QueryModel<T_AREA>("FCode", criminal.FAreaCode);

            string vouno = "QL" + DateTime.Now.ToString("yyyyMMddHHmmss");

            T_JF_Vcrd vcrd = new T_JF_Vcrd() {
                FCrimeCode = card.fcrimecode,
                FCriminal = criminal.FName,
                FAreaCode = criminal.FAreaCode,
                FAreaName = area.FName,
                AccType = 0,
                BankFlag = 0,
                BankInterfaceFlag = 0,
                bankRcvFlag = 0,
                CAmount = card.AccPoints,
                DAmount = 0,
                CardCode = card.cardcodea,
                CardType = 0,
                CheckBy = "",
                CheckDate = null,
                CheckFlag = 0,
            ...
            try
            {
                using (TransactionScope ts = new TransactionScope())
                {
                    ...
                    var rsVcrd = _jifenMgrService.Insert(vcrd);

                    //清空积分
                    card.AccPoints = 0;
                    if(card.bankdate < Convert.ToDateTime("1900-01-01"))
                    {
                        card.bankdate = Convert.ToDateTime("1900-01-01");
                    }
                    
                    _jifenMgrService.Update(card);


                    ts.Complete();
                    return Content("OK|清空成功|" + jss.Serialize(rsVcrd));
                }
            }
            catch (Exception ex)
            {
                return Content("Error|清空失败|" + ex.Message);
            }
        ...
        {
            string strFCode = Request["FCode"];
            List<T_JF_Vcrd> vcrds = new List<T_JF_Vcrd>();
            if (id == 1)
            {
                vcrds = _jifenMgrService.GetPageList<T_JF_Vcrd>( "Id asc", "",1,100,"FCrimecode='" + strFCode + "' and Flag in (0,-2) and TypeFlag=0").rows;

            }
            else if (id == 2)
            {
                vcrds = _jifenMgrService.GetPageList<T_JF_Vcrd>("Id asc","",1,100,"FCrimecode='" + strFCode + "' and Flag in (0,-2) and TypeFlag=2").rows;
            ...
            if (Request.Files.Count > 0)
            {
                ...
                using (FileStream stream = new FileStream(savePath, FileMode.Open, FileAccess.Read))
                {
                    ...
                    else
                    {
                        ...
                        {
                            FMoneyInOutFlag = 1;
                            subSaveTypes = new T_SavetypeBLL().GetModelList(" TypeFlag=0 and UseType=1 and FName='" + strDTypeName + "' and FCode=" + strDTypeCode + "");

                        }
                        else if (id == 2)
                        {
                            FMoneyInOutFlag = -1;
                            subSaveTypes = new T_SavetypeBLL().GetModelList(" TypeFlag=1 and UseType=1 and FName='" + strDTypeName + "' and FCode=" + strDTypeCode + "");
                        }
                        ...
                        string strFBid = _jifenMgrService.CreateOrderId("PLK");//批量扣款主单号
                        ...
                        dict.Add("strFBid", strFBid);
                        dict.Add("onlyCheckFlag", onlyCheckFlag);

                        string result = _jifenMgrService.ExecuteProc("JF_PLExcelFileImport", dict);
        ...
        public ActionResult ExcelOutSucList(int id=1)//Excel导出成功记录
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select fcrimecode 编号,fcriminal 姓名,FAmount 积分,Remark 备注 from T_BatchMoneyTrade_dtl ");
        ...
        public ActionResult ExcelOutErrList(int id = 1)//Excel导出失败记录
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select fcrimecode 编号,fname 姓名,Amount 积分,Remark 备注,notes 错误原因 from T_BatchMoneyTrade_ErrList ");
