...
{
    [LoginActionFilter]
    public class JfReportController : BaseController
    {
        ...
        public ActionResult Index(int id=1)
        {
            ...
            DataTable dt = new CommTableInfoBLL().GetDataTable("select distinct CrtBy from t_JF_vcrd Order by CrtBy");
            ...
            dt = new CommTableInfoBLL().GetDataTable("select distinct Dtype from t_JF_vcrd where damount<>0 Order by Dtype");
            ...
            dt = new CommTableInfoBLL().GetDataTable("select distinct Dtype from t_JF_vcrd where camount<>0 Order by Dtype");
        ...
        public ActionResult getInvList()
        {
            ...
            T_JF_Invoice invoice = _jifenMgrService.QueryModel<T_JF_Invoice>("InvoiceNo",invNo);
            if (invoice!=null)
            {
        ...
        public ActionResult SaveChangeType()
        {
            ...
            if (invoice != null)
            {
                if (_jifenMgrService.ChangeVcrdListType(invNo, saveTypes[0].fname, saveTypes[0].fcode))
                {
        ...
        public ActionResult GetSearchVcrds()
        {
            ...
            PageResult<T_JF_Vcrd> vcrds = _jifenMgrService.GetPageList<T_JF_Vcrd, T_JF_Vcrd>("CrtDate asc,FCrimeCode asc", "",page, row, strWhere);

            sss = "{\"total\":" + vcrds.total.ToString() + ",\"rows\":" + jss.Serialize(vcrds.rows) + ",\"sum\":"+ vcrds.rows.Sum(o=>o.DAmount-o.CAmount) + "}";
            return Content(sss);
        }
        ...
        public ActionResult PrintCriminalSumOrder(int id=1)
        {
            ...
            List<T_JF_Vcrd> vcrds;
            if (param != null)
            {
                vcrds = _jifenMgrService.QueryList<T_JF_Vcrd>(strSql.ToString(), param).ToList();
            }
            else
            {
                vcrds = _jifenMgrService.QueryList<T_JF_Vcrd>(strSql.ToString(),null);
            }
                ...
                case 1://按个用户个人汇总表
                    {
                        if(intFlag==0)
                        {
                            strSql.Append("select FCrimeCode,FCriminal,b.FAreaName,Sum(Damount) Damount,Sum(Camount) Camount,b.Remark from T_JF_VCrd a" +
                        ...
                        {
                            strSql.Append("select FCrimeCode 编号,FCriminal 姓名,b.FAreaName 队别,Sum(Damount) 收入,Sum(Camount) 支出,Sum(Damount-Camount) 剩余积分,b.Remark 备注 from T_JF_VCrd a" +
                                ",(Select FCode ,FAreaName=(select fname from t_area where fcode=e.FAreaCode),case when isnull(e.fflag,0)=1 then '离监' when isnull(e.fflag,0)=2 then '保外' else '在押' end as Remark from T_Criminal e ) b  ");
                        }
                ...
                case 2://按个用户个人清表
                    {
                        if(intFlag==0)
                        {
                            strSql.Append("select Top 5000 Vouno,FCrimeCode,FCriminal,Dtype,DAmount,CAmount,FAreaName,FAreaCode,CrtDate,BankFlag from T_JF_VCrd  ");
                        }
                        else
                        {
                            strSql.Append("select Top 200000 Vouno 流水号,FCrimeCode 编号,FCriminal 姓名,Dtype 类型,DAmount 收入,CAmount 支出,FAreaName 队别,FAreaCode 队别号,CrtDate 日期,BankFlag 银行标志,Origid 消费流水单号 from T_JF_VCrd  ");
                        }
                ...
                case 3://按队别汇总总表
                    {
                        if (intFlag == 0)
                        {
                            strSql.Append("select FAreaName,DType,Sum(Damount) Damount,Sum(Camount) Camount from T_JF_VCrd  ");
                        }
                        else
                        {
                            strSql.Append("select FAreaName 队别,DType 类型,Sum(Damount) 收入,Sum(Camount) 支出 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                ...
                case 4://按队别分类清表
                    {
                        if (intFlag == 0)
                        {
                            strSql.Append("select Top 5000 Vouno,FCrimeCode,FCriminal,Dtype,DAmount,CAmount,FAreaName,FAreaCode,CrtDate,BankFlag,Remark from T_JF_VCrd  ");
                        }
                        else
                        {
                            strSql.Append("select Top 200000 Vouno 流水号,FCrimeCode 编号,FCriminal 姓名,Dtype 类型,DAmount 收入,CAmount 支出,FAreaName 队别,FAreaCode 队别号,CrtDate 日期,BankFlag 银行标志,Origid 消费流水单号,Remark 备注 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                ...
                case 5://按存取类型汇总总表
                    {
                        if (intFlag == 0)
                        {
                            strSql.Append("select TypeFlag,DType,Sum(Damount) Damount,Sum(Camount) Camount from T_JF_VCrd  ");
                        }
                        else
                        {
                            strSql.Append("select TypeFlag 类型ID,DType 类型,Sum(Damount) 收入,Sum(Camount) 支出 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                ...
                case 7://超市消费汇总表
                    {
                        ...
                        {
                            strSql.Append("select a.FAreaCode 队别号,a.FAreaName 队别,a.Dtype 类型,a.Damount+isnull( b.Damount,0) 退货积分,a.Camount+isnull( b.Camount,0) 消费积分 from (");
                        }
                        strSql.Append("");
                        strSql.Append("select FAreaCode,FAreaName,Dtype,Sum(Damount) Damount,Sum(Camount) Camount from T_JF_VCrd  ");
                ...
                case 8://明细记录清单
                    {
                        ...
                        {
                            strSql.Append("select Top 200000 Vouno 流水号,FCrimeCode 编号,FCriminal 姓名,Dtype 类型,DAmount 收入,CAmount 支出,FAreaName 队别,FAreaCode 队别号,CrtDate 日期,BankFlag 银行标志,Remark 备注 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                ...
                case 9://按消费日期银行交易汇总报表
                    {
                        ...
                        strSql.Append("select Bankflag ,substring( Convert(varchar(20),SendDate,102),1,7) Remark,TypeFlag,DType,Sum(Damount) Damount,Sum(Camount) Camount from T_JF_VCrd  ");
                ...
                case 10://银行交易明细清单
                    {
                        if (intFlag == 0)
                        {
                            strSql.Append("select Top 8000 Vouno,FCrimeCode,FCriminal,Dtype,DAmount,CAmount,FAreaName,FAreaCode,CrtDate,SendDate,case Bankflag  when 2 then '成功' when 3 then '手动成功' when 4 then '手动成功' else  '失败' end Depositer  from T_JF_VCrd  ");
                        }
                        else
                        {
                            strSql.Append("select Top 200000 Vouno 流水号,FCrimeCode 编号,FCriminal 姓名,Dtype 类型,DAmount 收入,CAmount 支出,FAreaName 队别,FAreaCode 队别号,CrtDate 下单日期,SendDate 回款日期,BankFlag 银行标志 from T_JF_VCrd  ");
                        }
                        //strWhere = strWhere.Replace("CrtDate", "SendDate");
                        strSql.Append(" Where " + strWhere + " and TypeFlag not in(5,6,14) ");
                ...
                case 11://银行每日交易报表
                    {
                        if (intFlag == 0)
                        {
                            strSql.Append("select convert(datetime, convert(varchar(20), CrtDate,102)) CrtDate,SendDate,TypeFlag,DType,BankFlag,Sum(Damount) Damount,Sum(Camount) Camount from T_JF_VCrd  ");
                        }
                        else
                        {
                            strSql.Append("select convert(datetime, convert(varchar(20), CrtDate,102)) 消费日期,SendDate 交易日期,TypeFlag 类型ID,DType 类型,BankFlag 回款标志,Sum(Damount) 收入,Sum(Camount) 支出 from T_JF_VCrd  ");
                        }
                        //strWhere = strWhere.Replace("CrtDate", "SendDate");
                        strSql.Append(" Where " + strWhere + " and TypeFlag not in(5,6,14) ");
                ...
                case 12:
                    {
                        ...
                        else
                        {
                            strSql.Append(@"select fcrimecode 编号,fcriminal 姓名,CardCode 银行卡
                                ,FAreaName 队别,CAmount 剩余积分,CrtDate 日期,DType 类型,CrtBy 操作员 from (
                                select a.fcrimecode,c.fname fcriminal,b.BankAccNo CardCode,a.Crtby CrtBy,0 Flag
                                ,a.CrtDate CrtDate,0 DAmount,(a.AmountA+a.AmountB+a.AmountC) CAmount,d.FName FAreaName,'离监取款' DType
                                from t_balanceList a inner join t_Criminal_Card b on a.fcrimecode=b.fcrimecode 
                                inner join t_criminal c on a.fcrimecode=c.fcode
                ...
                case 13://按回款日期银行交易汇总报表
                    {
                        ...
                        strSql.Append("select Bankflag ,substring( Convert(varchar(20),CrtDate,102),1,7) Remark,TypeFlag,DType,Sum(Damount) Damount,Sum(Camount) Camount from T_JF_VCrd  ");
                ...
                case 15://按回款日期银行每日交易报表
                    {
                        if (intFlag == 0)
                        {
                            strSql.Append("select convert(datetime, convert(varchar(20), CrtDate,102)) crtdate,convert(datetime, convert(varchar(20), SendDate,102)) SendDate,TypeFlag,DType,BankFlag,Sum(Damount) Damount,Sum(Camount) Camount from T_JF_VCrd  ");
                        }
                        else
                        {
                            strSql.Append("select convert(datetime, convert(varchar(20), CrtDate,102)) 消费月份,convert(datetime, convert(varchar(20), SendDate,102)) 交易日期,TypeFlag 类型ID,DType 类型,BankFlag 回款标志,Sum(Damount) 收入,Sum(Camount) 支出 from T_JF_VCrd  ");
                        }
                        strWhere = strWhere.Replace("CrtDate", "SendDate");
                ...
                case 21://罚金清单打印
                    {
                        ...
                        if (intFlag == 0)
                        {
                            strSql.Append("select Top 5000 FAreaName,FCrimeCode,FCriminal,DAmount,CAmount,Dtype,Remark,FAreaCode=(select frealareaCode from t_criminal where fcode=t_JF_vcrd.fcrimecode),Vouno=(select f.fname from t_criminal e,t_crime f where e.fcrimecode=f.fcode and e.fcode=t_JF_vcrd.fcrimecode),CrtDate,BankFlag from t_JF_vcrd  ");
                        }
                        else
                        {
                            strSql.Append("select Top 5000 FAreaName 队别,FCrimeCode 狱号,FCriminal 姓名,DAmount 收,CAmount 支,Dtype 类型,Remark 用途,案件号=(select frealareaCode from t_criminal where fcode=t_JF_vcrd.fcrimecode),罪名=(select f.fname from t_criminal e,t_crime f where e.fcrimecode=f.fcode and e.fcode=T_JF_VCrd.fcrimecode),CrtDate 日期,BankFlag 扣款状态 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                ...
                case 22://存取类型个人汇总打印
                    {
                        ...
                        if (intFlag == 0)
                        {
                            strSql.Append("select Top 5000  fcrimecode,fcriminal,left(convert(varchar(20),crtdate,120),10) crtdate,dtype,fareaName,remark,sum(damount) DAmount,sum(camount) CAmount from T_JF_VCrd ");
                        }
                        else
                        {
                            //strSql.Append("select Top 5000 fcrimecode 编号,fcriminal 姓名,fareaName 队别,sum(damount-camount) 金额,remark 备注,dtype 类型 from T_JF_VCrd  ");
                            strSql.Append("select Top 5000 fcrimecode 编号,fcriminal 姓名,fareaName 队别,sum(damount-camount) 积分 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                ...
                case 23://存取类型个人汇总打印(+有日期)
                    {
                        ...
                        {
                            //strSql.Append("select Top 5000 fcrimecode 编号,fcriminal 姓名,fareaName 队别,sum(damount-camount) 金额,remark 备注,dtype 类型 from T_JF_VCrd  ");
                            strSql.Append("select Top 5000 left(convert(varchar(20),crtdate,120),10) 日期,fcrimecode 编号,fcriminal 姓名,fareaName 队别,sum(damount-camount) 积分 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                        strSql.Append(" group by fcrimecode,fcriminal,left(convert(varchar(20),crtdate,120),10),dtype,fareaName,fareacode,remark ");
                        strSql.Append(" order by left(convert(varchar(20),crtdate,120),10) ,fareacode,fcriminal");
                ...
                case 24://存取类型个人汇总打印
                    {
                        ...
                        {
                            strSql.Append("select Top 5000 left(convert(varchar(20),crtdate,120),10) 日期,fareaName 队别,fcrimecode 编号,fcriminal 姓名,dtype 类型,sum(damount-camount) 积分,remark 备注 from T_JF_VCrd  ");
                            //strSql.Append("select Top 5000 fcrimecode 编号,fcriminal 姓名,fareaName 队别,sum(damount-camount) 金额 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                        strSql.Append(" group by fcrimecode,fcriminal,left(convert(varchar(20),crtdate,120),10),dtype,fareaName,fareaCode,remark ");
                        strSql.Append(" order by left(convert(varchar(20),crtdate,120),10) ,fareaCode,fcriminal");
                ...
                case 25:
                    {
                        ...
                        if (intFlag == 0)
                        {
                            ...
                            strSql.Append("select Top 5000 FCrimeCode,FCriminal,Dtype,FAreaName,FAreaCode,convert(varchar(10), CrtDate,120) as CrtDate ,BankFlag,Remark from T_JF_VCrd  ");
                        }
                        else
                        {
                            strSql.Append("select b.*,a.AmountA,a.AmountB,a.AmountC,a.PayMode from t_balanceList a,(");
                            strSql.Append("select Top 200000 FCrimeCode 编号,FCriminal 姓名,Dtype 类型,FAreaName 队别,FAreaCode 队别号,convert(varchar(10), CrtDate,120) 日期,BankFlag 银行标志,Remark 备注 from T_JF_VCrd  ");
                        }
                        strSql.Append(" Where " + strWhere);
                ...
                case 26:
                    {
                        if (intFlag == 0)
                        {
                            strSql.Append(@"select a.fcrimecode,fcriminal,c.FName As FAreaName,case b.fflag when 1 then '离监' when 2 then '保外' else '在押' end as Frealareacode,sum(DAMOUNT-CAMOUNT) as DAmount
                                ...
                                from (select * from T_JF_VCrd where " + strWhere + @") a left outer join T_CRIMINAL b on a.fcrimecode=b.FCode
                                left outer join T_AREA c on b.FAreaCode=c.FCode 
                                left outer join (select fcrimecode,CONVERT(varchar(10), dateadd(day,1,CRTDATE),120) OutDate from T_JF_VCrd where flag=0 and typeflag=5
                        ...
                        {
                            strSql.Append(@"select a.fcrimecode as 编号, fcriminal as 姓名, c.FName As 队别,case b.fflag when 1 then '离监' when 2 then '保外' else '在押' end as 状态,sum(DAMOUNT - CAMOUNT) 积分
        ...
        public ActionResult DeleteBankflagFailVcrd(int id=1)
        {
            ...
            {
                T_JF_Vcrd vcrd = _jifenMgrService.GetModel<T_JF_Vcrd>( Convert.ToInt32(seqno));
                if (vcrd.BankFlag == -1)
                {
                    if(_jifenMgrService.DeleteDtlByVcrdSeqno(vcrd))
                    {
                        rs = "OK|删除成功";
                    }
                    else
