...
function loginCheck() {
    ...
    if (cardNo != "") {
        $.post('/SelfPayment/LoginCheck', { "managerCardNo": cardNo},
            function (data, status) {
                if ('success' != status) {
                    ...
                    if (data != null) {
                        var ss = data.split("|");
                        if (ss[0] == "OK") {
                            window.location.href = "/SelfPayment/Index?LoginFlag=LoginOK122342124121123131231122&UserName=" + ss[2] + "&managerCardNo=" + cardNo;
                        } else {
            ...
            function (data, status) {
                if ('success' != status) {
                    ...
                    if (data != null) {
                        var ss = data.split("|");
                        if (ss[0] == "OK") {
                            window.location.href = "/SelfPayment/Index?LoginFlag=LoginOK122342124121123131231122&UserName=" + ss[2] + "&managerCardNo=" + ss[1];
                        } else {
                            alert(data);
                        }
                    }
