...
$('#test').datagrid({
    ...
    striped: true,
    collapsible: true,
    url: '/JfConsumePrint/GetSearchInvoices/' + $("#typeId").val(),
    //sortName: 'GCODE',       
    columns: [[
        ...
        { field: 'Amount', title: '积分', width: 60, align: 'right' },
    ...
    onSelect: function (index, row) {
        laodInvoiceDetail(row.InvoiceNo);
    },
...
$('#details').datagrid({
    ...
    columns: [[
        { field: 'ck', checkbox: true },
        { field: 'INVOICENO', title: '单号', width: 120, sortable: true },
        { field: 'GCODE', title: '商品编码', width: 80, sortable: true },
        { field: 'GNAME', title: '商品名称', width: 80, sortable: true },
        { field: 'GTXM', title: '条码', width: 100, sortable: true },
        { field: 'SPShortCode', title: '简码', width: 100, sortable: true },
...
function laodInvoiceDetail(invoiceNo) {
    if (invoiceNo == "") {
        $.messager.alert("提示", "消费主单号不能为空");
        return false;
    }
    $.post("/JfConsumePrint/GetInvoiceDtlByNo/", { "invoiceNo": invoiceNo }, function (data, status) {
        if (status == "success") {
            ...
            if (rows.length > 0) {
                $.messager.confirm('提示', '记录一共有' + rows.length +'条，对吗？', function (r) {
                    if (r) {
                        ...
                        $.post("/JfConsumePrint/SalesOrderAllReturn", { "Invoices": selectRows }, function (data, status) {
                            if (status != "success") {
                                return false;
                            } else {
                                var words = data.split("|");
...
function btnSubmitTuihuo() {
    ...
    $.post("/JfConsumePrint/AddTuiHuoOrder", effectRow, function (data, status) {
        if ("success" == status) {
            var words = data.split("|");
            if (words[0] == "OK") {
                $("#winTuiHuo").window("close");
...
function PrintCustomers(e) {
    ...
    $.post("/JfConsumePrint/GetInvoices", {
        "invoices": selectRows
                    ...
                    for (var p = 1; p <= pages; p++) {
                        ...
                        if (p == pages) {
                            ...
                            xiaopiao = xiaopiao + "</tbody>"
                                + "</table>"
                                + "<hr />"
                                + "<span>消费合计：" + inv.invoice.Amount + "积分</span><hr/>"
                                + "<div style='font-size:" + (parseInt($("#xiaoPiaoFontSize").val()) + 1) + "px;'>剩余积分:" + inv.criminal.AccPoints + "积分</div>"
...
function PrintSumOrder(id) {
    ...
    window.open("/JfConsumePrint/PrintSumOrder/" + id + "?" + strWhere);
...
function ExcelSumOrder(id) {
    ...
    $.post("/JfConsumePrint/ExcelSumOrder/" + id, objWhere, function (data, status) {
        if (status != "success") {
            return false;
        } else {
            var words = data.split("|");
...
function printAllXiaofeiDan() {
    ...
    var strWhere = getStrSearchWhere();
    //alert(strWhere);
    window.open("/JfConsumePrint/PrintAllXiaofeiDan?" + strWhere);
}
...
function ExcelAllXiaofeiDan() {
    ...
    $.post("/JfConsumePrint/ExcelAllXiaofeiDan/1", objWhere, function (data, status) {
        if (status != "success") {
            return false;
        } else {
            var words = data.split("|");
    ...
    $.messager.confirm('提示', '您真要对执行撤单操作吗?', function (r) {
        if (r) {
            ...
            if (rows.length > 0) {
                ...
                $.post("/JfConsumePrint/CancelInvoiceOrder", { "Invoices": selectRows }, function (data, status) {
                    if (status != "success") {
                        return false;
                    } else {
                        var words = data.split("|");
