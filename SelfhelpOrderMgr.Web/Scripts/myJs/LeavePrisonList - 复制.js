...
function QuerenXianjinLingqu() {
    ...
    if (paymode != 0) {
        $.messager.alert("提示", "不是现金结算方式，无需确认");
        return false;
    }
    if (selected.CollectMoneyFlag == 1) {
        $.messager.alert("提示", "已经确认过了，无需重复确认");
        return false;
    }
    $.messager.confirm('确认框', '您确认该犯已从财务科领取现金，并继续执行标记?', function (r) {
        if (r) {
            $.post("/Infomgr/QuerenXianjinLingqu", {
                "FCode": selected.fcode,
                "FName": selected.fname,
