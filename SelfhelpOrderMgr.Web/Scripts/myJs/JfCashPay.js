...
function LoadPayOrder() {
    ...
    $('#test').datagrid({
        ...
        { field: 'FAreaName', title: '监区', width: 150 },
        {
            ...
            { field: 'AccPoints', title: '账户积分', width: 80 },
        ...
        toolbar: [{
            ...
            handler: function () {
                ...
                window.open("/JfCashPay/IndexExcelDR/"+ $("#saveTypeId").val());
            }
        }
        //, '-', {
        //    id: 'btnBatchPrint',
...
function loadDetailTable() {
    $('#detail').datagrid({
        ...
        striped: true,
        collapsible: true,
        url: '/JfCashPay/getVcrds/'+$("#saveTypeId").val(),
        sortName: 'Id',
        sortOrder: 'asc',
        remoteSort: false,
        idField: 'Id',
        ...
        columns: [[//DataGrid表格数据列
            ...
            { field: 'DAmount', title: '存入积分', width: 100 },
            { field: 'CAmount', title: '支出积分', width: 100 },
            {
...
function btnSaveDetail() {
    ...
    if ($("#txtMoney").val() == "") {
        ...
        $.post("/JfCashPay/SavePayDetail/" + $("#saveTypeId").val(), {
            "FCode": $("#txtFCode").val(),
            "FName": $("#txtFName").val(),
...
function DelDetailList() {
    ...
    if (row.BankFlag!=0) {
        ...
        $.messager.confirm('确认', '您是否真的要删除记录吗?', function (r) {
            if (r) {
                $.post("/JfCashPay/DelDetailList/" + $("#saveTypeId").val(), {
...
function PrintSavePayList(savePayFlag) {
    ...
    window.open("/JfCashPay/PrintSavePayList/" + $("#saveTypeId").val() + "?seqno=" + row.Id + "&FCode=" + row.FCode + "&savePayFlag=" + savePayFlag);
...
function ClearPoints()
{
    ...
    if (row == null) {
        $.messager.alert("提示", "请选择一条要清空的人员");
        return false;
    }

    $.messager.confirm('确认', '您是否真的要清空该用户的积分吗?', function (r) {
        if (r) {
            $.post("/JfCashPay/ClearPoints/", {
                "FCode": row.FCode,
                "remark": $("#txtRemark").val()
            }, function (data, status) {
                if ("success" != status) {
                    ...
                    if (words[0] == "OK") {
                        ...
                        $('#detail').datagrid('appendRow', vcrd);
