...
$(function () {
    ...
    $('#tbTrade').datagrid({
        ...
        height: 110,
        singleSelect: true,
        url: '/JfCashPay/GetTrades/' + $("#saveTypeId").val(),
        columns: [[
            { field: 'Bid', title: '批号', width: 100 },
            { field: 'FCourseType', title: '类型', width: 100 },            
            {
                field: 'FAmount', title: '积分', width: 100, sortable: true, formatter: function (value, row, index) {
                    if (row.FAmount == "0") {
                        return "数据检测";
                    } else {
                        return value;
    ...
    $('#tbSuccessList').datagrid({
        url: '/JfCashPay/GetBatchTradeDtls/' + $("#saveTypeId").val(),
        columns: [[
            { field: 'Bid', title: '批号', width: 100 },
            { field: 'FCrimeCode', title: '编号', width: 100 },
            { field: 'FCriminal', title: '姓名', width: 100 },
            { field: 'FAmount', title: '积分', width: 100 },
            { field: 'Remark', title: '备注', width: 100, align: 'right' }
        ]]
    });

    $('#tbErrList').datagrid({
        url: '/JfCashPay/GetBatchTradeErrs/' + $("#saveTypeId").val(),
        columns: [[
            { field: 'pc', title: '批号', width: 100 },
            { field: 'fcrimecode', title: '编号', width: 100 },
            { field: 'fname', title: '姓名', width: 100 },
            { field: 'Amount', title: '积分', width: 100 },
            { field: 'Remark', title: '备注', width: 100, align: 'right' },
...
function startDaoru(onlyCheckFlag) {
    ...
    $.messager.confirm('确认', '您确认想要校验导入Excel记录吗？', function (r) {
        if (r) {
            ...
            $('#ff').form({
                url: "/JfCashPay/ExcelInport/" + $("#saveTypeId").val() + "?onlyCheckFlag=" + onlyCheckFlag,
                onSubmit: function () {
                    // do some check    
...
function plDeleteByPKId(pkId) {
    $.messager.confirm('确认', '您确认要删除该主单的记录吗？', function (r) {
        if (r) {
            $.post("/JfCashPay/plDeleteByPKId/" + $("#saveTypeId").val(), { "pkId": pkId }, function (data, status) {
                if ("success" != status) {
                    return false;
                } else {
                    var words = data.split("|");
...
function outportSucList() {
    ...
    $.post("/JfCashPay/ExcelOutSucList/" + $("#saveTypeId").val(), { "strFBid": $("#strFBid").val() }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            var words = data.split("|");
...
function outportErrList() {
    ...
    $.post("/JfCashPay/ExcelOutErrList/" + $("#saveTypeId").val(), { "strFBid": $("#strFBid").val() }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            var words = data.split("|");
