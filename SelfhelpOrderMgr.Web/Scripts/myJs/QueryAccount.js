...
function QueryUserInfo(cardNo) {
    $("#lblInfo").html(cardNo);
    $.post("/UserQuery/SearchAccountInfo", { "FCardCode": cardNo, "checkFlag": "1"}, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            ...
            $("#tbodyAccount").html("");
            if (data.Flag == true) {
                $("#searchUserName").html(data.DataInfo.UserInfo.FName);
                var trs = "";
                for (var i = 0; i < data.DataInfo.rcvs.length; i++) {
                    trs = trs + "<tr><td>" + data.DataInfo.rcvs[i].fractName + "</td><td>" + data.DataInfo.rcvs[i].fractnactacn + "</td><td>" + data.DataInfo.rcvs[i].fractnibknum + "</td><td>" + data.DataInfo.rcvs[i].fractnibkname +"</td></tr>" ;
                }
                $("#tbodyAccount").append(trs);
            } 
