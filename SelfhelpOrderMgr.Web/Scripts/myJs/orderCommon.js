...
function getUserCardNo(saleSort) {
    ...
    $.post("/Shopping/GetCardInfo", { "userCode": userCode, "userPwd": userPwd }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
    ...
    for (var i = 0; i < gtypes.length; i++) {
        ...
        for (var j = 0; j < goods.length; j++) {
            if (goods[j].GTYPE == gtypes[i].Fcode) {
                ...
                html += "<span class='MoneyFontColor'><small class='MoneyFontColor'><span class='glyphicon glyphicon-grain' aria-hidden='true'></span></small> " + good.GDJ + " 元</span><br />";
...
function CancelCurrOrderInfo() {
    ...
    $.post("/Shopping/AddOrder", { "FCardCode": cardNo, "checkFlag": "0", "saleTypeId": $("#saleTypeId").val() }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            var words = data.split("|")
...
function SetAndDisplayCriminalInfo(words, saleSort) {
    ...
    $("#criminalCyInfo").html("您好，" + rts.FName + "<span class='label label-default'>" + rts.cyName + "</span>,可消费金额：<span class='label label-default' id='xiaofeiYuE'>" + rts.NoXiaofeimoney + "</span>,账户总余额：<span class='label label-default' id='okUseAllMoney'>" + rts.OkUseAllMoney + "</span>,本月已经消费：<span class='label label-warning' id='curMonthXFE'>" + rts.Xiaofeimoney + "</span>");
    
    $("#orderMoney").html(toDecimal2(rts.orderMoney));//订单金额    
    $("#submitMoney").html(toDecimal2(rts.orderMoney));//结算显示金额
    $("#curMonthXFE").html(toDecimal2(rts.Xiaofeimoney));//本月已经消金额
    $("#tbodyOrderList").empty();
...
function btnDelDetailById(detailId, saleSort) {
    ...
    $.post("/Shopping/DelOrderDetail", { "OrderId": orderId, "Id": detailId }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            var words = data.split("|")
...
function DisplayPayModel(mode) {
    ...
    var option;
    $("#tbodyJieShuanList").empty();
    $.post("/Shopping/GetOrderDetails", { "orderId": $("#orderNumber").val() }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            var words = data.split("|");
...
function PayCheckSubmit() {
    ...
    var printFlag = "";
    $.post("/Shopping/GetPrintXiaoPiaoFlag", { "OrderId": "" }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            if (data == "1") {
    ...
    $.post("/Shopping/SubmitOrder", { "OrderId": $("#orderNumber").val(), "FCrimeCode": fcrimecode, "userRoomNo": $("#userRoomNO").val(), "crtby": "crtby" }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            var words = data.split("|");
...
function DoInsertBuyDetail(iallMoney, nowYue, saleSort) {
    ...
    $.post("/Shopping/AddOrderDetail", { "OrderId": $("#orderNumber").val(), "GTXM": gtxm, "GCount": $("#inputGcount").val(), "goodRemark": gRemark }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            var words = data.split("|");
...
function SetPrintPayXiaopiao(words) {
    ...
    if (inv.details.length > 20) {
        ...
        + "<div style='font-size:" + (parseInt($("#xiaoPiaoFontSize").val()) + 1) + "px;'>蓝风铃余额:" + inv.criminal.AmountD + "元</div>"
            + "<div style='font-size:" + (parseInt($("#xiaoPiaoFontSize").val()) + 1) + "px;'>账户总余额:" + toDecimal2(inv.criminal.AmountA + inv.criminal.AmountB + inv.criminal.AmountC + inv.criminal.AmountD) + "元</div>"
        + "</div><br/></div>";
