...
function btnBuy(e) {
    ...
    $.post("/JfShopping/GetGoodsStockNumber", { "GTXM": words[0] }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
...
function btnGoodsSearch() {
    ...
    $.post("/JfShopping/SearchGoodsInfo", { "saleTypeId": $("#saleTypeId").val(), "Gtxm": $("#gtxmSearchBox").val() }, function (data, status) {
        if ("success" != status) {
            return false;
        } else {
            var words = data.split("|")
