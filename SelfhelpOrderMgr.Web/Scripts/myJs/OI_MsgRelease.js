...
$(document).ready(function () {
    ...
    //$.post("/Message/GetCyTypeInfo", { "dt": new Date() }, function (data, status) {
    //    if ("success" == status) {
    ...
    //    }
    //});
    //$.post("/Message/GetAreaInfo", { "dt": new Date() }, function (data, status) {
    //    if ("success" == status) {
    //        for (var i = 0; i < data.length; i++) {
...
function btnDelRole() {
    ...
    if (confirm("确定删除该通知信息吗?")) {
        //点击确定后操作
        $.post("/Message/DeleteNotify", { "ID": $("#ID").val() }, function (data, status) {
            if ("success" == status) {
