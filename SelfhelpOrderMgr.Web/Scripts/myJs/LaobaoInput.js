$(function () {
    ...
    $('#test').datagrid({
        ...
        striped: true,
        collapsible: true,
        url: '/Laobao/getLaobaoDetailByBid',
        sortName: 'seqno',
        sortOrder: 'asc',
        remoteSort: false,
        idField: 'seqno',
...
function DelDetailList(seqno, idx) {
    $.post("/Laobao/DelOrderDetail", {  "seqno": seqno, "sbid": $("#sbid").val() }, function (data, status) {
        if (status == "success") {
...
function DelAllDetailList(bid) {
    $.post("/Laobao/DelAllOrderDetail", { "sbid": bid }, function (data, status) {
        if (status == "success") {
            var words = data.split(".");
            if (words[0] == "OK") {
                //更新主单数量及金额                
...
function InDbMain(bid) {
    ...
    if (row.Fdbcheckflag == 1) {
        if (row.FLAG == 0) {
            ...
            $.post("/Laobao/InDbMainOrder", { "sbid": row.BID }, function (data, status) {
                if (status == "success") {
...
function DeleteMainOrder(bid, idx) {
    $.post("/Laobao/DelMainOrder", {"sbid": bid }, function (data, status) {
        if (status == "success") {
            //alert(data);
            var words = data.split(".");
            if (words[0] == "OK") {
...
function loadMainOrderTable() {
    $('#mainOrderTable').datagrid({
        ...
        striped: true,
        collapsible: true,
        url: '/Laobao/getBonus/'+$("#PowerId").val(),
        sortName: 'BID',
        sortOrder: 'asc',
        remoteSort: false,
        idField: 'BID',
        ...
        { field: 'fAMOUNT', title: '总金额', width: 100 },
...
function undoYuanYingTJ() {
    ...
    $.post("/Laobao/undoYuanYingTJ/" + $("#PowerId").val(), {
        "Bid": okRow.BID,
...
function UndoOrder(bid, idx) {
    $.post("/Laobao/UndoMainOrder", { "sbid": bid }, function (data, status) {
        if (status == "success") {
            //alert(data);
            var words = data.split(".");
            if (words[0] == "OK") {
...
function CreateAreaList() {
    $.post("/Laobao/CreateAreaList", { "sbid": $("#sbid").val() }, function (data, status) {
        if (status == "success") {
            var words = data.split("|");
            if (words[0] == "OK") {
                //$.messager.alert('提示', "删除成功");
...
function BatchSaveDg() {
    ...
    if ($dg.datagrid('getChanges').length) {
        ...
        $.post("/Laobao/BatchSaveDtlList", effectRow,
            function (data, status) {
                if ("success" != status) {
                    return false;
                }
